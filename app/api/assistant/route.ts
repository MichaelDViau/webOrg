...
const FALLBACK_REPLY = `Sorry, I can't help with that one. For anything else, email ${site.email} or book a free consultation at /contact.`;

// Built once per server instance; identical text on every request keeps the prompt cache warm.
...
export async function POST(request: Request) {
  if (!process.env.ANTHROPIC_API_KEY) return badRequest("The assistant is not available right now.", 503);

  // Only accept requests from this site's own pages.
  const origin = request.headers.get("origin");
  if (!origin || new URL(origin).host !== request.headers.get("host")) {
    return badRequest("Requests must come from this website.", 403);
  }

  if (!rateLimit(`assistant:${await clientIp()}`, { limit: 20, windowMs: 10 * 60 * 1000 })) {
    return badRequest("You've sent a lot of messages in a short time. Please try again in a few minutes.", 429);
  }

  const messages = parseMessages(await request.json().catch(() => null));
  if (!messages) return badRequest("That message couldn't be sent. Please try a shorter question.");
    ...
    async start(controller) {
      ...
      try {
        ...
        if (!request.signal.aborted) {
          ...
          if (!wroteText) {
            controller.enqueue(
              encoder.encode(`Sorry, something went wrong on our side. Please try again, or email ${site.email}.`),
            );
          }
        }
      } finally {
